...
public class UniversityServiceImpl implements UniversityService {
    ...
    @Override
    @Transactional
    public String uploadPhoto(Long id, MultipartFile file, String userEmail) {
        ...
        if (user.getRole().getName() == RoleType.UNIVERSITY_ADMIN) {
            if (user.getUniversity() == null || !user.getUniversity().getId().equals(id)) {
                throw new AccessDeniedException("You can only upload photo for your own university");
            }
        } else if (user.getRole().getName() != RoleType.MAIN_ADMIN) {
            throw new AccessDeniedException("Access denied");
        }

        String oldPhotoPath = university.getPhotoUrl();
        if (oldPhotoPath != null && !oldPhotoPath.isBlank()) {
            try {
                storageService.delete(oldPhotoPath);
            } catch (Exception e) {
                log.warn("Could not delete old photo: {}", oldPhotoPath, e);
            }
        }

        String objectName = storageService.upload(file, "universities/" + university.getSlug());

        university.setPhotoUrl(objectName);
        universityRepository.save(university);

        return resolvePhotoUrl(objectName);
    }
    ...
    private String resolvePhotoUrl(String photoPath) {
        ...
        if (photoPath.startsWith("http")) return photoPath; // Already a URL

        try {
            return storageService.getFileUrl(photoPath);
        } catch (Exception e) {
            log.error("Failed to resolve file URL for path: {}", photoPath, e);
            return null;
        }
    }
    ...
    private void updateLeadership(University university, List<UpdateUniversityRequest.LeadershipDto> leadershipList) {
        ...
        Set<Long> incomingIds = leadershipList.stream().map(UpdateUniversityRequest.LeadershipDto::id).filter(Objects::nonNull).collect(Collectors.toSet());
        university.getLeadership().removeIf(l -> l.getId() != null && !incomingIds.contains(l.getId()));
        Map<Long, Leadership> existingMap = university.getLeadership().stream().filter(l -> l.getId() != null).collect(Collectors.toMap(Leadership::getId, Function.identity()));
    ...
    private void updateAchievements(University university, List<UpdateUniversityRequest.AchievementDto> achievementList) {
        ...
        Set<Long> incomingIds = achievementList.stream().map(UpdateUniversityRequest.AchievementDto::id).filter(Objects::nonNull).collect(Collectors.toSet());
        university.getAchievements().removeIf(a -> a.getId() != null && !incomingIds.contains(a.getId()));
        Map<Long, Achievement> existingMap = university.getAchievements().stream().filter(a -> a.getId() != null).collect(Collectors.toMap(Achievement::getId, Function.identity()));
    ...
    private void updateHistoryEvents(University university, List<UpdateUniversityRequest.HistoryEventDto> historyList) {
        ...
        Set<Long> incomingIds = historyList.stream().map(UpdateUniversityRequest.HistoryEventDto::id).filter(Objects::nonNull).collect(Collectors.toSet());
        university.getHistoryEvents().removeIf(h -> h.getId() != null && !incomingIds.contains(h.getId()));
        Map<Long, HistoryEvent> existingMap = university.getHistoryEvents().stream().filter(h -> h.getId() != null).collect(Collectors.toMap(HistoryEvent::getId, Function.identity()));
    ...
    private void updateFaculties(University university, List<UpdateUniversityRequest.FacultyDto> facultyList) {
        ...
        Set<Long> incomingIds = facultyList.stream().map(UpdateUniversityRequest.FacultyDto::id).filter(Objects::nonNull).collect(Collectors.toSet());
        university.getFaculties().removeIf(f -> f.getId() != null && !incomingIds.contains(f.getId()));
        Map<Long, Faculty> existingMap = university.getFaculties().stream().filter(f -> f.getId() != null).collect(Collectors.toMap(Faculty::getId, Function.identity()));
    ...
    private void updateFacultyTranslations(Faculty faculty, Map<String, UpdateUniversityRequest.FacultyTranslationDto> translations) {
        if (translations == null) return;
        Map<String, FacultyTranslation> existingMap = faculty.getTranslations().stream().collect(Collectors.toMap(t -> t.getLanguage().getCode(), Function.identity()));
        for (Map.Entry<String, UpdateUniversityRequest.FacultyTranslationDto> entry : translations.entrySet()) {
            ...
            if (translation == null) {
                Language language = languageRepository.findByCode(langCode).orElseThrow(() -> new RuntimeException("Language not found: " + langCode));
    ...
    private void updateDepartments(Faculty faculty, List<UpdateUniversityRequest.DepartmentDto> departmentList) {
        ...
        Set<Long> incomingIds = departmentList.stream().map(UpdateUniversityRequest.DepartmentDto::id).filter(Objects::nonNull).collect(Collectors.toSet());
        faculty.getDepartments().removeIf(d -> d.getId() != null && !incomingIds.contains(d.getId()));
        Map<Long, Department> existingMap = faculty.getDepartments().stream().filter(d -> d.getId() != null).collect(Collectors.toMap(Department::getId, Function.identity()));
    ...
    private void updateDepartmentTranslations(Department department, Map<String, UpdateUniversityRequest.DepartmentTranslationDto> translations) {
        if (translations == null) return;
        Map<String, DepartmentTranslation> existingMap = department.getTranslations().stream().collect(Collectors.toMap(t -> t.getLanguage().getCode(), Function.identity()));
    ...
    private void updateProgramGroups(Faculty faculty, List<UpdateUniversityRequest.EducationalProgramGroupDto> groupList) {
        ...
        Set<Long> incomingIds = groupList.stream().map(UpdateUniversityRequest.EducationalProgramGroupDto::id).filter(Objects::nonNull).collect(Collectors.toSet());
        faculty.getProgramGroups().removeIf(g -> g.getId() != null && !incomingIds.contains(g.getId()));
        Map<Long, EducationalProgramGroup> existingMap = faculty.getProgramGroups().stream().filter(g -> g.getId() != null).collect(Collectors.toMap(EducationalProgramGroup::getId, Function.identity()));
        Map<Long, Department> departmentMap = faculty.getDepartments().stream().filter(d -> d.getId() != null).collect(Collectors.toMap(Department::getId, Function.identity()));
    ...
    private void updateProgramGroupTranslations(EducationalProgramGroup group, Map<String, UpdateUniversityRequest.ProgramGroupTranslationDto> translations) {
        if (translations == null) return;
        Map<String, EducationalProgramGroupTranslation> existingMap = group.getTranslations().stream().collect(Collectors.toMap(t -> t.getLanguage().getCode(), Function.identity()));
    ...
    private void updatePrograms(EducationalProgramGroup group, List<UpdateUniversityRequest.EducationalProgramDto> programList) {
        ...
        Set<Long> incomingIds = programList.stream().map(UpdateUniversityRequest.EducationalProgramDto::id).filter(Objects::nonNull).collect(Collectors.toSet());
        group.getPrograms().removeIf(p -> p.getId() != null && !incomingIds.contains(p.getId()));
        Map<Long, EducationalProgram> existingMap = group.getPrograms().stream().filter(p -> p.getId() != null).collect(Collectors.toMap(EducationalProgram::getId, Function.identity()));
    ...
    private void updateProgramTranslations(EducationalProgram program, Map<String, UpdateUniversityRequest.ProgramTranslationDto> translations) {
        if (translations == null) return;
        Map<String, EducationalProgramTranslation> existingMap = program.getTranslations().stream().collect(Collectors.toMap(t -> t.getLanguage().getCode(), Function.identity()));
    ...
    private void updateTuitionDiscounts(University university, List<UpdateUniversityRequest.TuitionDiscountDto> discountList) {
        ...
        Set<Long> incomingIds = discountList.stream().map(UpdateUniversityRequest.TuitionDiscountDto::id).filter(Objects::nonNull).collect(Collectors.toSet());
        university.getTuitionDiscounts().removeIf(d -> d.getId() != null && !incomingIds.contains(d.getId()));
        Map<Long, TuitionDiscount> existingMap = university.getTuitionDiscounts().stream().filter(d -> d.getId() != null).collect(Collectors.toMap(TuitionDiscount::getId, Function.identity()));
    ...
    private void updateInternationalSections(University university, List<UpdateUniversityRequest.InternationalSectionDto> sectionList) {
        ...
        Set<Long> incomingIds = sectionList.stream().map(UpdateUniversityRequest.InternationalSectionDto::id).filter(Objects::nonNull).collect(Collectors.toSet());
        university.getInternationalSections().removeIf(s -> s.getId() != null && !incomingIds.contains(s.getId()));
        Map<Long, InternationalSection> existingMap = university.getInternationalSections().stream().filter(s -> s.getId() != null).collect(Collectors.toMap(InternationalSection::getId, Function.identity()));
    ...
    private void updateInternationalSectionTranslations(InternationalSection section, Map<String, UpdateUniversityRequest.InternationalSectionTranslationDto> translations) {
        if (translations == null) return;
        Map<String, InternationalSectionTranslation> existingMap = section.getTranslations().stream().collect(Collectors.toMap(t -> t.getLanguage().getCode(), Function.identity()));
    ...
    private void updateInternationalItems(InternationalSection section, List<UpdateUniversityRequest.InternationalItemDto> itemList) {
        ...
        Set<Long> incomingIds = itemList.stream().map(UpdateUniversityRequest.InternationalItemDto::id).filter(Objects::nonNull).collect(Collectors.toSet());
        section.getItems().removeIf(i -> i.getId() != null && !incomingIds.contains(i.getId()));
        Map<Long, InternationalItem> existingMap = section.getItems().stream().filter(i -> i.getId() != null).collect(Collectors.toMap(InternationalItem::getId, Function.identity()));
    ...
    private void updateInternationalItemTranslations(InternationalItem item, Map<String, UpdateUniversityRequest.InternationalItemTranslationDto> translations) {
        if (translations == null) return;
        Map<String, InternationalItemTranslation> existingMap = item.getTranslations().stream().collect(Collectors.toMap(t -> t.getLanguage().getCode(), Function.identity()));
    ...
    private int calculateProgress(University university) {
        ...
        long completeTranslations = university.getTranslations().stream().filter(t -> t.getName() != null && !t.getName().isBlank() && t.getCity() != null && !t.getCity().isBlank() && t.getDescription() != null && !t.getDescription().isBlank()).count();
    ...
    private UniversityDetailResponse.ProgressDto calculateDetailedProgress(University university) {
        ...
        long namesCount = university.getTranslations().stream().filter(t -> t.getName() != null && !t.getName().isBlank()).count();
        if (namesCount == 3) basicFilled++;
        long citiesCount = university.getTranslations().stream().filter(t -> t.getCity() != null && !t.getCity().isBlank()).count();
        ...
        int totalPercent = basicPercent + descPercent + leaderPercent + achievePercent + facultyPercent + admissionPercent + tuitionPercent + intlPercent;
        return new UniversityDetailResponse.ProgressDto(totalPercent, new UniversityDetailResponse.SectionProgress("basicInfo", basicPercent, 20, basicFilled, basicTotal), new UniversityDetailResponse.SectionProgress("description", descPercent, 15, descFilled, descTotal), new UniversityDetailResponse.SectionProgress("leadership", leaderPercent, 10, leaderFilled, leaderTotal), new UniversityDetailResponse.SectionProgress("achievements", achievePercent, 10, achieveFilled, achieveTotal), new UniversityDetailResponse.SectionProgress("faculties", facultyPercent, 20, facultyFilled, facultyTotal), new UniversityDetailResponse.SectionProgress("admissionRules", admissionPercent, 10, admissionFilled, admissionTotal), new UniversityDetailResponse.SectionProgress("tuition", tuitionPercent, 5, tuitionFilled, tuitionTotal), new UniversityDetailResponse.SectionProgress("international", intlPercent, 10, intlFilled, intlTotal));
    }
    ...
    private UniversityDetailResponse.FacultyDto mapFacultyToDto(Faculty faculty) {
        ...
        for (FacultyTranslation t : faculty.getTranslations()) {
            translations.put(t.getLanguage().getCode(), new UniversityDetailResponse.FacultyTranslationDto(t.getName(), t.getDescription()));
        }
        List<UniversityDetailResponse.DepartmentDto> departments = faculty.getDepartments().stream().sorted(Comparator.comparingInt(d -> d.getSortOrder() != null ? d.getSortOrder() : 0)).map(this::mapDepartmentToDto).toList();
        List<UniversityDetailResponse.EducationalProgramGroupDto> programGroups = faculty.getProgramGroups().stream().sorted(Comparator.comparingInt(g -> g.getSortOrder() != null ? g.getSortOrder() : 0)).map(this::mapProgramGroupToDto).toList();
        return new UniversityDetailResponse.FacultyDto(faculty.getId(), faculty.getSortOrder(), translations, departments, programGroups);
    }
    ...
    private UniversityDetailResponse.DepartmentDto mapDepartmentToDto(Department department) {
        ...
        for (DepartmentTranslation t : department.getTranslations()) {
            translations.put(t.getLanguage().getCode(), new UniversityDetailResponse.DepartmentTranslationDto(t.getName(), t.getGoal(), t.getMission(), t.getTasks()));
        }
    ...
    private UniversityDetailResponse.EducationalProgramGroupDto mapProgramGroupToDto(EducationalProgramGroup group) {
        ...
        for (EducationalProgramGroupTranslation t : group.getTranslations()) {
            translations.put(t.getLanguage().getCode(), new UniversityDetailResponse.ProgramGroupTranslationDto(t.getName(), t.getDescription()));
        }
        List<UniversityDetailResponse.EducationalProgramDto> programs = group.getPrograms().stream().sorted(Comparator.comparingInt(p -> p.getSortOrder() != null ? p.getSortOrder() : 0)).map(this::mapProgramToDto).toList();
        UniversityDetailResponse.PassingScoreDto passingScoreDto = null;
        if (group.getPassingScore() != null) {
            var ps = group.getPassingScore();
            passingScoreDto = new UniversityDetailResponse.PassingScoreDto(ps.getMinScoreGrant(), ps.getMinScorePaid(), ps.getProfileSubjects(), ps.getIsCreativeExam());
        }
        return new UniversityDetailResponse.EducationalProgramGroupDto(group.getId(), group.getDepartment() != null ? group.getDepartment().getId() : null, group.getDegreeLevel().name(), group.getCode(), group.getSortOrder(), translations, programs, passingScoreDto);
    }
    ...
    private UniversityDetailResponse.EducationalProgramDto mapProgramToDto(EducationalProgram program) {
        ...
        for (EducationalProgramTranslation t : program.getTranslations()) {
            translations.put(t.getLanguage().getCode(), new UniversityDetailResponse.ProgramTranslationDto(t.getName(), t.getDescription()));
        }
        return new UniversityDetailResponse.EducationalProgramDto(program.getId(), program.getCode(), program.getSortOrder(), translations);
    }
    ...
    private UniversityDetailResponse.InternationalSectionDto mapInternationalSectionToDto(InternationalSection section) {
        ...
        for (InternationalSectionTranslation t : section.getTranslations()) {
            translations.put(t.getLanguage().getCode(), new UniversityDetailResponse.InternationalSectionTranslationDto(t.getTitle(), t.getDescription()));
        }
        List<UniversityDetailResponse.InternationalItemDto> items = section.getItems().stream().sorted(Comparator.comparingInt(i -> i.getSortOrder() != null ? i.getSortOrder() : 0)).map(this::mapInternationalItemToDto).toList();
        return new UniversityDetailResponse.InternationalSectionDto(section.getId(), section.getExternalUrl(), section.getSortOrder(), section.getIsActive(), translations, items);
    }
    ...
    private UniversityDetailResponse.InternationalItemDto mapInternationalItemToDto(InternationalItem item) {
        ...
        for (InternationalItemTranslation t : item.getTranslations()) {
            translations.put(t.getLanguage().getCode(), new UniversityDetailResponse.InternationalItemTranslationDto(t.getTitle(), t.getDescription()));
        }
        return new UniversityDetailResponse.InternationalItemDto(item.getId(), item.getExternalUrl(), item.getSortOrder(), item.getIsActive(), translations);
    }
}