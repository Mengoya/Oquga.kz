...
export function FacultiesSection({ data, onChange }: Props) {
    const t = useTranslations('UniversityEdit.faculties');
    const tCommon = useTranslations('Common.actions');
    const tLevels = useTranslations('UniversityEdit.degreeLevels');
    ...
    const degreeLevels = [
        ...
        { value: 'DOCTORATE', label: tLevels('doctorate') },
    ];
    ...
    const [expandedProgramGroup, setExpandedProgramGroup] = useState<
        string | null
    >(null);
    ...
    const updateFacultyTranslation = (
        facultyIndex: number,
        lang: string,
        field: string,
        value: string,
    ...
    const updateDepartment = (
        facultyIndex: number,
        deptIndex: number,
        updates: Partial<DepartmentDto>,
    ) => {
        const faculty = faculties[facultyIndex];
        updateFaculty(facultyIndex, {
            departments: faculty.departments.map((d, i) =>
                i === deptIndex ? { ...d, ...updates } : d,
            ),
        });
    };

    const updateDepartmentTranslation = (
        facultyIndex: number,
        deptIndex: number,
        lang: string,
        field: string,
    ...
    const removeProgramGroup = (facultyIndex: number, groupIndex: number) => {
        const faculty = faculties[facultyIndex];
        updateFaculty(facultyIndex, {
            programGroups: faculty.programGroups.filter(
                (_, i) => i !== groupIndex,
            ),
        });
    };

    const updateProgramGroup = (
        facultyIndex: number,
        groupIndex: number,
        updates: Partial<EducationalProgramGroupDto>,
    ) => {
        const faculty = faculties[facultyIndex];
        updateFaculty(facultyIndex, {
            programGroups: faculty.programGroups.map((g, i) =>
                i === groupIndex ? { ...g, ...updates } : g,
            ),
        });
    };

    const updateProgramGroupTranslation = (
        facultyIndex: number,
        groupIndex: number,
        lang: string,
        field: string,
    ...
    const updateProgram = (
        ...
        groupIndex: number,
        progIndex: number,
        updates: Partial<EducationalProgramDto>,
    ) => {
        const faculty = faculties[facultyIndex];
        const group = faculty.programGroups[groupIndex];
        updateProgramGroup(facultyIndex, groupIndex, {
            programs: group.programs.map((p, i) =>
                i === progIndex ? { ...p, ...updates } : p,
            ),
        });
    };

    const updateProgramTranslation = (
        facultyIndex: number,
        groupIndex: number,
        progIndex: number,
        lang: string,
    ...
    const updatePassingScore = (
        facultyIndex: number,
        groupIndex: number,
        updates: Partial<PassingScoreDto> | null,
    ) => {
        const faculty = faculties[facultyIndex];
        const group = faculty.programGroups[groupIndex];
        if (updates === null) {
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="font-medium">
                                    {faculty.translations.ru?.name ||
                                        t('facultyNumber', {
                                            number: facultyIndex + 1,
                                        })}
                                </span>
                                <span className="text-xs text-muted-foreground">
                                    ({faculty.departments.length} каф.,{' '}
                                    {faculty.programGroups.length} гр. прог.)
                                </span>
                            </div>
                            <Button
                                variant="ghost"
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div
                                                            className="flex items-center justify-between p-3 cursor-pointer"
                                                            onClick={() =>
                                                                setExpandedProgramGroup(
                                                                    expandedProgramGroup ===
                                                                        groupKey
                                                                        ? null
                                                                        : groupKey,
                                                                )
