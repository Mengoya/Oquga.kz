...
export function TuitionSection({ data, onChange }: Props) {
    const t = useTranslations('UniversityEdit.tuition');
    ...
    const updateDiscount = (
        index: number,
        updates: Partial<TuitionDiscountDto>,
    ) => {
        onChange({
            tuitionDiscounts: tuitionDiscounts.map((d, i) =>
                i === index ? { ...d, ...updates } : d,
            ),
        });
    };
            ...
            <div className="space-y-4">
                {tuitionDiscounts.map((discount, index) => (
                    ...
                    >
                        ...
                        <div>
                            <Label>{t('scholarshipInfo')}</Label>
                            <Textarea
                                value={discount.scholarshipInfo || ''}
                                onChange={(e) =>
                                    updateDiscount(index, {
                                        scholarshipInfo: e.target.value || null,
                                    })
                                }
                                rows={3}
